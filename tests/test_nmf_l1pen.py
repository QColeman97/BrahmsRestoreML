...
class RestorePenaltyTests(unittest.TestCase):    
    ...
    def test_restore_brahms_l1pen_pianoh(self):
        ...
        spectrogram, phases = dsp.make_spectrogram(sig, nmf.PIANO_WDW_SIZE, ova=True, debug=debug_flag)
        
        # Pos. learn index = learn piano
        activations, basis_vectors = nmf.nmf(spectrogram, (nmf.NUM_PIANO_NOTES + num_noise_bv_test), basis_vectors=given_basis_vectors, 
                                               learn_index=(-1 * num_noise_bv_test), madeinit=True, debug=debug_flag, l1_penalty=l1_penalty_test)
        # Compare to no l1-Penalty
        non_pen_activations, non_pen_basis_vectors = nmf.nmf(spectrogram, (nmf.NUM_PIANO_NOTES + num_noise_bv_test), basis_vectors=given_basis_vectors, 
