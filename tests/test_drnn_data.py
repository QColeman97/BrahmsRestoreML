...
from brahms_restore_ml.drnn.drnn import TRAIN_SEQ_LEN, TRAIN_FEAT_LEN, MIN_SIG_LEN
import unittest
import numpy as np
...
class DRNNDataTests(unittest.TestCase):
    ...
    def test_preprocess_sig(self):
        ...
        min_sig_len = 5
        sig_mix, sig1, sig2 = preprocess_signals(sig1, sig2, min_sig_len)
        with self.subTest():
            self.assertEqual(len(sig_mix), min_sig_len)
        with self.subTest():
            self.assertEqual(len(sig1), min_sig_len)
        with self.subTest():
            self.assertEqual(len(sig2), min_sig_len)

    def test_generator(self):
        n_samples, bs, n_seq, n_feat, min_sig_len = 1, 1, TRAIN_SEQ_LEN, TRAIN_FEAT_LEN, MIN_SIG_LEN
        ...
        gen = nn_data_generator(y1_files, y2_files, n_samples, bs, n_seq, n_feat, 
                                min_sig_len=min_sig_len, dmged_piano_artificial_noise=False,
                                data_path=data_path)
        yielded = next(gen)
        with self.subTest():
    ...
    def test_generator_from_numpy(self):
        n_samples, bs, n_seq, n_feat, min_sig_len = 2, 1, TRAIN_SEQ_LEN, TRAIN_FEAT_LEN, MIN_SIG_LEN
        ...
        y2_files = [data_path+'noise_source_numpy/noise'+str(i)+'.npy' 
                    for i in range(n_samples)]
        gen = nn_data_generator(y1_files, y2_files, n_samples, bs, n_seq, n_feat,
                                min_sig_len=min_sig_len, x_files=x_files, from_numpy=True)
        yielded = next(gen)
        with self.subTest():
            self.assertEqual(len(yielded), 2)
        with self.subTest():
    ...
    def test_get_features_stats(self):
        n_samples, n_seq, n_feat, min_sig_len = 6, TRAIN_SEQ_LEN, TRAIN_FEAT_LEN, MIN_SIG_LEN
        x_files = [data_path+'piano_noise_numpy/mixed'+str(i)+'.npy' 
                    for i in range(n_samples)]
        y1_files = [data_path+'piano_source_numpy/piano'+str(i)+'.npy' 
                    for i in range(n_samples)]
        y2_files = [data_path+'noise_source_numpy/noise'+str(i)+'.npy' 
                    for i in range(n_samples)]
        mean, std = get_features_stats(y1_files, y2_files, n_samples, n_seq, n_feat, 
                min_sig_len, data_path=data_path, x_filenames=x_files, from_numpy=True)
        with self.subTest():
