...
def make_best_dft(waveform):
    # Note: best_wdw_num is naturally-indexed
...
# basis_vectors = np.array(basis_vectors).T     # Needed? Yes
...
# V LOGIC
# Brahms recording: wave_filepath = brahms_filepath
...
# # print('Original Sig:\n', bm_sig[piano_wdw_size * (bm_num_wdws // 2): (piano_wdw_size * (bm_num_wdws // 2)) + 10])
...
# # Spectrogram matrix w/ correct orientation
# spectrogram = np.array(spectrogram).T     # Needed? Yes
...
# H LOGIC
# Learn / Approximate Activation Matrix
# Main dimensions: freq bins = spectrogram.shape[1], piano keys (components?) = num_notes, windows = bm_num_wdws
...
# print('Synthetic Sig:\n', synthetic_sig[piano_wdw_size * (bm_num_wdws // 2): (piano_wdw_size * (bm_num_wdws // 2)) + 10])
