...
def grid_search(y1_train_files, y2_train_files, y1_val_files, y2_val_files,
                ...
                early_stop_pat=3, data_path='./dlnn_data/', pc_run=False):
    ...
    # TEST
    # get_stats(y1_train_files, y2_train_files, num_train,
    ...
    if pc_run == 'True':
        ...
        # with open(data_path + 'hp_arch_config_largedim.json') as hp_file:
        with open(data_path + 'hp_arch_config_nodimreduc.json') as hp_file:
            bare_config_optns = json.load(hp_file)['archs']

    combos = (len(batch_size_optns) * len(epochs_optns) * len(loss_const_optns) *
              len(optimizer_optns) * len(dropout_optns) * len(scale_optns) *
              len(rnn_skip_optns) * len(bias_rnn_optns) * len(bias_dense_optns) * 
              len(bidir_optns) * len(bn_optns) * len(rnn_optns) * len(bare_config_optns))

    # Comment-out block for all-factors version below
            ...
            for loss_const in loss_const_optns:
                for opt, clip_val, lr, opt_name in optimizer_optns:
                    for arch_config in arch_config_optns:
                        ...
                        proc_str = '_testproc_' if tf.config.experimental.get_memory_growth(gpus[0]) else '_mainproc_'
                        with open(data_path + 'grid_search_results/result_' + str(gs_iter) + '_of_' + str(combos) + proc_str + str(os.getpid()) + '.txt', 'w') as w_fp:
...
def main():
    # PROGRAM ARGUMENTS #
    if len(sys.argv) < 3:
        print('\nUsage: dlnn_brahms_restore.py <mode> <PC>')
        print('Parameter Options:')
        print('Mode     TRAIN_INFER     - Train model, then restore brahms with model')
        print('         GRID_SEARCH     - Perform grid search')
        print('         INFER_ONLY      - Restore brahms with last-trained model')
    ...
    if mode == 'INFER_ONLY':
        ...
        if mode == 'TRAIN_INFER':
            ...
            dropout_optns = [(0.0,0.0)]
            arch_config_optns = []   # Add variations of each bare config to official
            for config in bare_config_optns[0:1]:    # rand base = #71 last
