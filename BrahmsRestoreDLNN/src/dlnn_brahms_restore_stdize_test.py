...
# TEST - 2 GS's at same time? SUCCESS!! Just runs a lot slower
...
def get_stats(y1_filenames, y2_filenames, num_samples, train_seq, train_feat, 
    ...
    for i in range(num_samples):
        pl_sr, piano_label_sig = wavfile.read(y1_filenames[i])
        nl_sr, noise_label_sig = wavfile.read(y2_filenames[i])
        # pl_orig_type, nl_orig_type = piano_label_sig.dtype, noise_label_sig.dtype 
        piano_label_sig, noise_label_sig = piano_label_sig.astype('float64'), noise_label_sig.astype('float64')
        assert len(noise_label_sig) == len(piano_label_sig)  
        # Stereo audio safety check
...
def evaluate_source_sep(train_generator, validation_generator,
                        ...
                        patience=10, epsilon=10 ** (-10), config=None,
                        t_mean=None, t_std=None, grid_search_iter=None):
    # print('X shape:', X.shape, 'y1 shape:', y1.shape, 'y2 shape:', y2.shape)
    # print('X shape:', X.shape)
    print('Making model...')
    # print('DEBUG Batch Size in Eval Src Sep:', batch_size)
    
    # TEST failed - distribute across shared mem?
    ...
    #                        ('./grid_search_models/model' + str(grid_search_iter) + '.png'
    ...
    # avg_loss = np.nanmean(hist.history['loss'])
    # avg_val_loss = np.nanmean(hist.history['val_loss'])
...
# def grid_search(train_generator, validation_generator, n_feat, n_seq):
def grid_search(y1_train_files, y2_train_files, y1_val_files, y2_val_files,
                n_feat, n_seq, wdw_size, epsilon, max_sig_len, early_stop_pat=3,
                data_path='./dlnn_data/'):
    ...
    # 10 maybe 20 combinations for architecture
    # 20 * 16 * 7 * 3 * 3 (* 2)
    # 20,000 combos (40,000 combos)

    # Data aug combos
    # 2 * 2 * 2 (* 2) = 8 combos
    # bad/good piano, add static, freq aug, time stretch/shrinkange 
    ...
    batch_size_list = [5, 9, 15]    # IMPORTANT: Factors of total samples * (1 - val_split) for performance
    epochs_list = [10, 50, 100]
    loss_const_list = [0, 0.05, 0.1, 0.15, 0.2]#, 0.25, 0.3]
    optimizer_list = [(tf.keras.optimizers.RMSprop(), 0, 0.001, 'RMSprop')]#, 
                      #(tf.keras.optimizers.RMSprop(clipvalue=0.25), 0.25), 
                      #(tf.keras.optimizers.RMSprop(clipvalue=0.5), 0.5)]#, 
                      #(tf.keras.optimizers.RMSprop(clipvalue=0.75), 0.75)]
    # Optimizers - 16 combos? RMSprop, Adam,

    # Optional - for future when I'm not hitting SNR correctly
    # amp_var_rng_list = [(0.5, 1.25), (0.75, 1.15), (0.9, 1.1)]

    with open(data_path + 'hp_arch_config.json') as hp_file:
        bare_config_list = json.load(hp_file)['archs']

    # dropout_optns = [(0.0,0.0), (0.2,0.2), (0.2,0.5), (0.5,0.2), (0.5,0.5)]
    # arch_config_list = []   # Add variations of each bare config to official
    # for config in bare_config_list:
    #     for scale_optn in [True, False]:    # TODO: Be skeptical of scaling, b/c must scale at end before activation
    #         for rnn_skip_optn in [True, False]:
    #             for bias_rnn_optn in [True, False]:
    #                 for bias_dense_optn in [True, False]:
    #                     for dropout_optn in dropout_optns:      # For RNN only
    #                         for bidir_optn in [True, False]:
    #                             for bn_optn in [True, False]:   # For Dense only
    #                                 for rnn_optn in ['RNN', 'LSTM']:
    ...
    #                                     arch_config_list.append(curr_config)       
    
    # TEST - some factors version below
    # batch_size_list = [5]    # IMPORTANT: Factors of total samples * (1 - val_split) for performance
    # CUDA_OUT_OF_MEMORY Error Debug
    batch_size_list = [3]    # IMPORTANT: Factors of total samples * (1 - val_split) for performance
    epochs_list = [10]
    loss_const_list = [0.05]
    optimizer_list = [(tf.keras.optimizers.RMSprop(clipvalue=0.9), 0.9, 0.001, 'RMSprop')]     
    dropout_optns = [(0.0,0.0)]
    arch_config_list = []   # Add variations of each bare config to official
    for config in bare_config_list[:1]:
        # TEMP - 2 tests @ once: 1 for hpc test, 2 for diff scalinng methods
        for scale_optn in [True, False]:    # TODO: Be skeptical of scaling, b/c must scale at end before activation
            for rnn_skip_optn in [False]:
                for bias_rnn_optn in [True]:
                    for bias_dense_optn in [True]:
                        for dropout_optn in dropout_optns:      # For RNN only
                            for bidir_optn in [False]:
                                for bn_optn in [False]:   # For Dense only
    ...
    for batch_size in batch_size_list:
        for epochs in epochs_list:
            for loss_const in loss_const_list:
                for opt, clip_val, lr, opt_name in optimizer_list:
                    for arch_config in arch_config_list:
                        ...
                        # Not supported yet (Python 3.4 on HPC)
                        ...
                        proc_str = '_testproc' if tf.config.experimental.get_memory_growth(gpus[0]) else '_mainproc'
                        with open(data_path + 'grid_search_results/result' + str(gs_iter) + proc_str + '.txt', 'w') as w_fp:
                        ...
                        print('DONE W/ GRID-SEARCH ITER', (str(gs_iter) + ':'), 
                            batch_size, epochs, loss_const, opt, clip_val, lr, arch_config, '\n')
                        gs_iter += 1
...
def infer(X, phases, wdw_size, model, loss_const, optimizer, 
    ...
    #plot_matrix(clear_spgm, name='Clear Spectrogram', ylabel='Frequency (Hz)', 
    #            ratio=SPGM_BRAHMS_RATIO)
    #plot_matrix(noise_spgm, name='Noise Spectrogram', ylabel='Frequency (Hz)', 
    #            ratio=SPGM_BRAHMS_RATIO)
...
def main():
    ...
    epsilon = 10 ** (-10)
    loss_const, epochs, train_batch_size, val_split = 0.05, 10, 5, 0.25 #(1/3) - makes everything divisible (5,10,20 batch_size)
    ...
    if mode == 'INFER_ONLY':
        ...
        test_filepath = ((data_path + 'brahms.wav'))
        ...
        try:
            ...
            print(model.summary())
            restore_audio_file(output_path, model, wdw_size, epsilon,
                            loss_const, optimizer, test_filepath)
    else:
        ...
        # TODO: Mix in code, Dont use noise_piano_filepath at all AND test freq_perturb
        # (Only used for testing on synthetic data - right now)
        # noisy_piano_filepath_prefix = ((data_path + 'train_val_data_dmged/features')
        #     if use_dmged_piano or use_artificial_noise else 
        #     (data_path + 'train_val_data/features'))
        ...
        if mode == 'TRAIN_INFER':
            ...
            training_samples, validation_samples = len(y1_train_files), len(y1_val_files)
