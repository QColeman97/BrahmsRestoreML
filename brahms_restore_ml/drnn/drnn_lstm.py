...
# Use only once in program, or use cxt for multiple calls per program (at forking time)
...
def evaluate_source_sep(# train_dataset, val_dataset,
            ...
            x_val_files, y1_val_files, y2_val_files,
                        ...
                        patience=100, epsilon=10 ** (-10), config=None, recent_model_path=None, pc_run=False,
                        t_mean=None, t_std=None, grid_search_iter=None, gs_path=None, combos=None, gs_id='',
                        keras_fit=False, ret_queue=None, dataset2=False):
    ...
    if ret_queue is not None:
        # print('Putting return values into results queue:', ret_queue, ret_queue.qsize())
        # ret_queue.put(history['loss'])
        # print('Queue with losses:', ret_queue, ret_queue.qsize())
        # ret_queue.put(history['val_loss'])
        # print('Queue with validation losses:', ret_queue, ret_queue.qsize())
...
def get_hp_configs(bare_config_path, pc_run=False):
    ...
    # NEEDS Gradient Clipping to get non-NaN loss values, experiment first to find best clip-val, start w/ 10
...
def grid_search(x_train_files, y1_train_files, y2_train_files, 
                ...
                gs_id='', restart=False, keras_fit=False, dataset2=False):
    ...
    # mirrored_strategy = None if pc_run else tf.distribute.MirroredStrategy()
    ...
    # For multiprocessing return values
    # q = multiprocessing.Queue()
    # send_end, recv_end = multiprocessing.Pipe()   # moved inside GS loop
    # print('Empty queue for multiprocess:', q, q.qsize())
                ...
                for loss_const in loss_const_optns:
                    for clip_val, lr, opt_name in optimizer_optns:
                        ...
                        if restart or (gs_iter > last_done):
                            ...
                            # MOVED INSIDE EVAL_SRC_SEP FOR MULTIPROC
                            # train_generator = fixed_data_generator(
                            #         x_train_files, y1_train_files, y2_train_files, num_train,
                            #         batch_size=batch_size, num_seq=n_seq, num_feat=n_feat, pc_run=pc_run)
                            # validation_generator = fixed_data_generator(
                            ...
                            #                                         combos=combos,
                            #                                         keras_fit=keras_fit,
                            #                                         dist_strat=mirrored_strategy)
                            
                            # ctx = multiprocessing.get_context('spawn')
                            # This hung, will adding context work?
                            send_end, recv_end = multiprocessing.Pipe()
                            # send_end, recv_end = ctx.Pipe()
                            ...
                            # process_train = ctx.Process(target=evaluate_source_sep, args=(

                                                                    # GEN MAKING MOVED INSIDE EVAL_SRC_SEP FOR MULTIPROC
                                                                    x_train_files, y1_train_files, y2_train_files, 
                                                                    x_val_files, y1_val_files, y2_val_files,
                                                                    ...
                                                                    gsres_path,
                                                                    combos, gs_id,
                                                                    keras_fit, send_end, dataset2))# q,))
                            # RECOVER FROM FAIL IN CHILD
                            # try:
                            #     process_train.start()
                            #     # # print('Getting ret vals from training')
                            #     # losses = q.get()
                            #     # # print('Losses from q:', losses)
                            #     # val_losses = q.get()
                            #     # print('Val. losses from q:', val_losses)
                            #     losses, val_losses = recv_end.recv()
                            #     print('Losses from pipe:', losses)
                            #     print('Val. losses from pipe:', val_losses)
                            #     process_train.join()
                            # except Exception as e:
                            #     print('\nERROR happened in child:', e)
                            #     exit(1)
                            ...
                            # Keep polling until child success or time is up (child prob died)
                            #   If child fails, need to know... (so poll for 8 mins max (6 min should be longest))
                            # i, timeout, losses, val_losses = 0, 8, None, None
                            # while (losses is None) and (i < timeout):
                            ...
                            # print('Queue size after train run:', q.qsize())
    ...
    if mode == 'r':
        ...
        if mode == 't':
            ...
            model, _, _ = evaluate_source_sep(#train_generator, validation_generator, 
                                    ...
                                    # Gen making now in evaluate_source_sep
                                    x_train_files, y1_train_files, y2_train_files, 
                                    x_val_files, y1_val_files, y2_val_files,
                                    ...
                                    keras_fit=keras_fit, dataset2=dmged_piano_artificial_noise_mix)
