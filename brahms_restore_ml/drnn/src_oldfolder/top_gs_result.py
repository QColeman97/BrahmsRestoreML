...
def min_file_and_value(results_path, num_iters, val_loss=False):
    min_file, min_value = None, None

    for i in range(1, num_iters + 1):
        file_name = results_path + 'result_' + str(i) + '_of_' + str(num_iters) + '_noPC.txt'
        try:
...
def files_close_to_min(within, min_val, results_path, num_iters, val_loss=False):
    within_min_files, within_min_values = [], []
    # within = 0.25 # 0.1
    # value_limit_vl = min_val + (min_val * within_vl)
    value_limit = min_val + (min_val * within)
...
def main():
    ...
    if num_gs_iters == 3072:
        ...
        grid_search_results_path = '../output_grid_search_lstm/'
    ...
    min_vl_file, min_val_loss = min_file_and_value(grid_search_results_path, num_gs_iters, val_loss=True)
    min_l_file, min_loss = min_file_and_value(grid_search_results_path, num_gs_iters)

    print('Min val. loss:', min_val_loss, min_vl_file)
    print('Min loss:', min_loss, min_l_file)

    min_vl_files, min_vl_values = files_close_to_min(within_vl, min_val_loss, grid_search_results_path, num_gs_iters, val_loss=True)
    min_l_files, min_l_values = files_close_to_min(within_l, min_loss, grid_search_results_path, num_gs_iters)

    with open('../top_gs_results_vl_' + str(num_gs_iters) + '.txt', 'w') as fp:
        for i, file_num in enumerate(min_vl_files):
            line_string = str(file_num) + ', ' + str(min_vl_values[i]) + '\n'
            fp.write(line_string)
    with open('../top_gs_results_l_' + str(num_gs_iters) + '.txt', 'w') as fp:
