...
def signal_to_pos_mag_fft(sgmt, wdw_size, ova=False, debug_flag=False):
...
# SIGNAL -> SPECTROGRAM
def signal_to_pos_mag_fft_old(sgmt, wdw_size, ova=False, debug_flag=False):
    if len(sgmt) != wdw_size:
        deficit = wdw_size - len(sgmt)
        sgmt = np.pad(sgmt, (0,deficit))  # pads on right side (good b/c end of signal), (deficit, 0) pads on left side # , mode='constant')
...
def make_spectrogram(signal, wdw_size, ova=False, debug=False):
    ...
    for i in range(num_sgmts):
        ...
        pos_mag_fft, pos_phases_fft = signal_to_pos_mag_fft(sgmt, wdw_size, ova=ova, debug_flag=debug_flag)
    ...
    if debug:
        # TEMP
        plot_matrix(spectrogram, 'Built Spectrogram', 'time segments', 'frequency', ratio=SPGM_BRAHMS_RATIO)
...
def make_spectrogram_old(signal, wdw_size, epsilon, ova=False, debug=False, hop_size_divisor=2):
    ...
    # Incorrect
    # num_sgmts = (int(((num_spls/wdw_size) - (1-(1/hop_size_divisor))) * hop_size_divisor) 
    #             if ova else math.ceil(num_spls / wdw_size))
    ...
    for i in range(num_sgmts):
        ...
        # print('.......................................................')
        # print('Mags:', pos_mag_fft[:10], 'Phases:', pos_phases_fft[:10])
...
def pos_mag_fft_to_signal(pos_mag_fft, pos_phases_fft, wdw_size, ova=False, end_sig=None, debug_flag=False):
...
# Returns real signal, given positive magnitude & phases of a DFT
# TEMP
def pos_mag_fft_to_signal_old(pos_mag_fft, pos_phases_fft, wdw_size, ova=False, 
...
def make_synthetic_signal(synthetic_spgm, phases, wdw_size, orig_type, ova=False, debug=False, return_16bit=False):
    ...
    for i in range(num_sgmts):
        ...
        if ova and len(synthetic_sig):
            synthetic_sgmt = pos_mag_fft_to_signal(pos_mag_fft=synthetic_spgm[i], pos_phases_fft=phases[i], 
            ...
            synthetic_sig += pos_mag_fft_to_signal(pos_mag_fft=synthetic_spgm[i], pos_phases_fft=phases[i], 
                                                       wdw_size=wdw_size, ova=ova, debug_flag=debug_flag)
...
# TEMP
# Construct synthetic waveform
def make_synthetic_signal_old(synthetic_spgm, phases, wdw_size, orig_type, ova=False, debug=False, hop_size_divisor=2):
...
def convert_sig_8bit_to_16bit(sig):
    print('IN CONVERT 8bit->16bit, sig:', sig[:10])
    sig = sig.astype('int16')
    print('sig as type 16bit:', sig[:10])
    sig = sig - 128     # Bring to range [-128, 127]
    print('sig - 128:', sig[:10])
...
def convert_sig_16bit_to_8bit(sig, safe_gaurd=True):
    ...
    if safe_gaurd:
        sig = sig.astype('int16')
        print('sig as type int 16:', sig[:10])
    sig = sig + 128     # Bring to range [0, 255]
    print('sig + 128:', sig[:10])
    # TEMP
    sig = sig.astype('uint8')
    print('sig as type uint8:', sig)
    print('EXITING CONVERT 8bit->16bit')
    return sig
...
# TEMP
# Badly named function, actually converts from type output by DSP (float64)
def convert_sig_16bit_to_8bit_old(sig):
    sig = sig / 256     # Bring to range [-128, 127]
    # sig = sig / 32768   # Bring to range [-1.0, 0.99] ~ [-1.0, 1.0]
    # sig = sig * 128     # Bring to range [-128, 127]
