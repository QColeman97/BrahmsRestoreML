...
def shrink_tsteps(in_path, out_path, shrink_to=100, noise=False):
    out_i = 0
    for i in range(61):
        sr, sig = wavfile.read(in_path + ('n' if noise else 'p') + 'source' + str(i) + '.wav')
        sig_type = sig.dtype
        ...
        while t_step_index < spgm.shape[0]:
            ...
            out_name = out_path + ('n' if noise else 'p') + 'source' + str(out_i) + '.wav'
...
in_path = data_path + 'artificial_noise_data/'
out_path = data_path + 'small_art_noise_data/'
shrink_tsteps(in_path, out_path, noise=True)
